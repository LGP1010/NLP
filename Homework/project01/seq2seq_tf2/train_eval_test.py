...
def train(params):
    ...
    # model.ckpt-200.meta文件保存的是图结构，通俗地讲就是神经网络的网络结构
    # model.ckpt-200.data-00000-of-00001：数据文件，保存的是网络的权值，偏置，操作等等
    # model.ckpt-200.index是一个不可变得字符串表，每一个键都是张量的名称，它的值是一个序列化的BundleEntryProto
    # checkpoint是检查点文件，文件保存了一个目录下所有的模型文件列表
    checkpoint_dir = "{}/checkpoint".format(params["seq2seq_model_dir"])
    ckpt = tf.train.Checkpoint(SequenceToSequence=model) # 实例化Checkpoint，指定恢复对象为model
    ckpt_manager = tf.train.CheckpointManager(ckpt, checkpoint_dir, max_to_keep=5) # max_to_keep 为保留的 Checkpoint 数目

    ckpt.restore(ckpt_manager.latest_checkpoint) # 从文件恢复模型参数
