...
def build(train_x_seg_path, test_y_seg_path, test_seg_path, out_path=None, sentence_path='',
    ...
    w2v = （one line）
    """
    w2v = Word2Vec(LineSentence(sentence_path), size=256, sg=1, min_count=1) # LineSentence把txt文件转为所需要的格式(已经分词，以空格隔开)
    ...
    # load model
    model = KeyedVectors.load_word2vec_format(w2v_bin_path, binary=True) # #加载训练的词向量
    word_dict = {}
